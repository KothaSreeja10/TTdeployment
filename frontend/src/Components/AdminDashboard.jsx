...
const StatCard = ({ icon, label, value, accent }) => (
  <div style={{
    background: 'rgba(30,20,50,0.7)', border: `1px solid ${accent}33`,
...
const AdminBtn = ({ onClick, children, variant = 'primary', disabled }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...styles[variant],
      padding: '9px 18px', borderRadius: '9px', fontWeight: 700,
      fontSize: '0.83rem', border: 'none', cursor: disabled ? 'not-allowed' : 'pointer',
...
const Input = ({ label, ...props }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
    ...
    <input {...props} style={{
      ...
      width: '100%', outline: 'none', fontFamily: 'inherit', fontSize: '0.88rem',
      ...props.style,
    }} />
  </div>
)
...
const Select = ({ label, children, ...props }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
    {label && <label style={{ fontSize: '0.78rem', color: '#a78bfa', fontWeight: 600, letterSpacing: '0.04em' }}>{label}</label>}
    <select {...props} style={{
      background: 'rgba(20,12,40,0.9)', border: '1px solid rgba(139,92,246,0.2)',
      borderRadius: '8px', color: '#f1f5f9', padding: '10px 14px',
      width: '100%', outline: 'none', fontFamily: 'inherit', fontSize: '0.88rem',
    }}>{children}</select>
  </div>
...
const Alert = ({ type, children }) => (
  <div style={{
    padding: '11px 16px', borderRadius: '8px', fontSize: '0.85rem', marginBottom: '16px',
...
const Modal = ({ title, onClose, children }) => (
  <div style={{
    ...
    <div style={{
      background: '#1a0e2e', border: '1px solid rgba(139,92,246,0.25)',
...
export default function AdminDashboard() {
  ...
  const [modal,      setModal]      = useState(null)  // 'addCompany'|'addJob'|'addJobSkill'|'addSkill'|'viewStudent'
  ...
  const submitJob = async (e) => {
    e.preventDefault(); setLoading(true)
    try {
      await addJobRole(jbForm.companyId, {
        title: jbForm.title, salaryLpa: parseFloat(jbForm.salaryLpa),
        minCgpa: parseFloat(jbForm.minCgpa), minPassoutYear: parseInt(jbForm.minPassoutYear),
        description: jbForm.description,
      })
  ...
  const submitJobSkill = async (e) => {
    e.preventDefault(); setLoading(true)
    try {
      await addSkillToJob(jsForm.jobId, jsForm.skillId, parseFloat(jsForm.weightage))
      flash('✅ Skill added to job!'); closeModal(); setJsForm({ jobId: '', skillId: '', weightage: '' }); loadAll()
    } catch (err) { flash('❌ ' + (err.response?.data?.message || 'Failed'), 'error') }
    setLoading(false)
  }
  ...
  const submitSkill = async (e) => {
    e.preventDefault(); setLoading(true)
    try {
      await addSkill(skForm)
      flash(`✅ Skill "${skForm.name}" added!`); setSkForm({ name: '', category: '' }); loadAll()
      if (modal === 'addSkill') closeModal()
  ...
  /* ── derived stats ─── */
  const readyCount  = students.filter(s => ['JOB_READY', 'HIGHLY_COMPETITIVE'].includes(s.readinessLevel)).length
  const topCount    = students.filter(s => s.readinessLevel === 'HIGHLY_COMPETITIVE').length
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 20% 0%, #1a0533 0%, #0f172a 55%)', color: '#f1f5f9' }}>
      ...
      <nav style={{
        ...
        background: 'rgba(15,10,30,0.92)', borderBottom: '1px solid rgba(139,92,246,0.15)',
        backdropFilter: 'blur(20px)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ fontSize: '1.4rem' }}>🛡️</span>
          <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: '1.1rem', color: '#fff' }}>
            PlacementPro <span style={{ color: '#a78bfa', fontSize: '0.75rem', fontWeight: 600 }}>ADMIN</span>
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div>
              <div style={{ fontSize: '0.82rem', fontWeight: 600, color: '#fff' }}>Admin</div>
              <div style={{ fontSize: '0.7rem', color: '#a78bfa' }}>Super Admin</div>
            </div>
          </div>
          <button onClick={() => { removeAdmin(); navigate('/admin') }} style={{
      ...
      <div style={{ paddingTop: '80px', padding: '80px 32px 40px', maxWidth: '1280px', margin: '0 auto' }}>
        ...
        {tab === 'overview' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: '16px', marginBottom: '32px' }}>
              ...
              <StatCard icon="🏆" label="Highly Competitive" value={topCount}          accent="#a78bfa" />
            ...
            <div style={{
              ...
              <h3 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: '#fff', marginBottom: '18px' }}>
                ⚡ Quick Actions
              </h3>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: 'rgba(124,58,237,0.08)' }}>
                      {['Name', 'Branch', 'CGPA', 'Score', 'Status'].map(h => (
                        <th key={h} style={{ padding: '11px 16px', textAlign: 'left', fontSize: '0.75rem', color: '#a78bfa', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {students.slice(0, 6).map(s => (
                      <tr key={s.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                        <td style={{ padding: '11px 16px', fontWeight: 600, color: '#fff' }}>{s.fullName}</td>
                        ...
                        <td style={{ padding: '11px 16px' }}>
                          <Pill color={levelColor[s.readinessLevel] || '#64748b'}>{s.readinessLevel?.replace('_', ' ') || 'N/A'}</Pill>
                        </td>
                      </tr>
                    ))}
                    {students.length === 0 && (
                      <tr><td colSpan={5} style={{ textAlign: 'center', color: '#64748b', padding: '28px' }}>No students registered yet.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        {tab === 'companies' && (
          <>
            <SectionHeader title="🏢 Companies">
              <AdminBtn onClick={() => setModal('addCompany')}>+ Add Company</AdminBtn>
            </SectionHeader>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'rgba(124,58,237,0.1)' }}>
                    {['Company', 'Industry', 'Location', 'HR Email', 'Website'].map(h => (
                      <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '0.75rem', color: '#a78bfa', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {companies.map(c => (
                    <tr key={c.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td style={{ padding: '13px 16px', fontSize: '0.82rem' }}>
                        {c.website ? <a href={c.website} target="_blank" rel="noreferrer" style={{ color: '#a78bfa' }}>🔗 Link</a> : '—'}
                      </td>
                    </tr>
                  ))}
                  {companies.length === 0 && <tr><td colSpan={5} style={{ textAlign: 'center', color: '#64748b', padding: '32px' }}>No companies yet. Click "+ Add Company"</td></tr>}
                </tbody>
              </table>
            </div>
          </>
          ...
          <>
            ...
            <div style={{ background: 'rgba(30,20,50,0.7)', border: '1px solid rgba(139,92,246,0.15)', borderRadius: '16px', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'rgba(124,58,237,0.1)' }}>
                    {['Job Title', 'Company', 'Salary', 'Min CGPA', 'Passout', 'Required Skills'].map(h => (
                      <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '0.75rem', color: '#a78bfa', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {jobs.map(j => (
                    <tr key={j.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td style={{ padding: '13px 16px', color: '#10b981', fontWeight: 700 }}>₹{j.salaryLpa} LPA</td>
                      <td style={{ padding: '13px 16px', color: '#f1f5f9' }}>{j.minCgpa}</td>
                      <td style={{ padding: '13px 16px', color: '#f1f5f9' }}>{j.minPassoutYear}</td>
                      <td style={{ padding: '13px 16px' }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                          {(j.requiredSkills || []).map(rs => (
                            <Pill key={rs.id} color="#10b981">{rs.skill?.name} ({rs.weightage}%)</Pill>
                          ))}
                          {(!j.requiredSkills || j.requiredSkills.length === 0) && (
                            <span style={{ color: '#64748b', fontSize: '0.8rem' }}>None — add skills</span>
                          )}
                        </div>
                      </td>
                    </tr>
                  ))}
                  {jobs.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', color: '#64748b', padding: '32px' }}>No job roles yet. Click "+ Add Job Role"</td></tr>}
                </tbody>
              </table>
            </div>
          </>
        ...
        {tab === 'skills' && (
          <>
            <SectionHeader title="🔧 Skills Library">
              <AdminBtn onClick={() => setModal('addSkill')}>+ Add Skill</AdminBtn>
            </SectionHeader>
            <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px' }}>
              ...
              <div style={{ background: 'rgba(30,20,50,0.8)', border: '1px solid rgba(139,92,246,0.18)', borderRadius: '16px', padding: '24px' }}>
                ...
                <form onSubmit={submitSkill} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  <Input label="SKILL NAME *" placeholder="e.g. Java, SQL, DSA" value={skForm.name} onChange={e => setSkForm({ ...skForm, name: e.target.value })} required />
                  <Select label="CATEGORY" value={skForm.category} onChange={e => setSkForm({ ...skForm, category: e.target.value })}>
                ...
                <div style={{ marginTop: '20px' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {['Java', 'SQL', 'DSA', 'Spring Boot', 'Python', 'React', 'Docker', 'AWS', 'Communication', 'Microservices', 'System Design'].map(sk => (
                      <button key={sk} onClick={() => quickAddSkill(sk)} style={{
              ...
              {/* skills grid */}
              <div style={{ background: 'rgba(30,20,50,0.7)', border: '1px solid rgba(139,92,246,0.15)', borderRadius: '16px', overflow: 'hidden' }}>
                <div style={{ padding: '16px 22px', borderBottom: '1px solid rgba(139,92,246,0.12)' }}>
                  <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: '#fff' }}>All Skills ({skills.length})</span>
                </div>
          ...
          <>
            <SectionHeader title="👨‍🎓 All Students">
              <div style={{ display: 'flex', gap: '8px' }}>
                <Pill color="#10b981">Top: {topCount}</Pill>
                <Pill color="#3b82f6">Ready: {students.filter(s => s.readinessLevel === 'JOB_READY').length}</Pill>
                <Pill color="#f59e0b">Mid: {students.filter(s => s.readinessLevel === 'INTERMEDIATE').length}</Pill>
                <Pill color="#ef4444">Begin: {students.filter(s => s.readinessLevel === 'BEGINNER').length}</Pill>
              </div>
            </SectionHeader>
            <div style={{ background: 'rgba(30,20,50,0.7)', border: '1px solid rgba(139,92,246,0.15)', borderRadius: '16px', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'rgba(124,58,237,0.1)' }}>
                    {['Name', 'Email', 'Branch', 'CGPA', 'Score', 'Status', 'Actions'].map(h => (
                      <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '0.75rem', color: '#a78bfa', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {students.map(s => (
                    <tr key={s.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.15s' }}
                      ...
                      <td style={{ padding: '12px 16px', fontWeight: 700, color: '#fff' }}>{s.fullName}</td>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <button onClick={() => openStudent(s)} style={{
                          ...
                          <button onClick={() => handleDeleteStudent(s.id)} style={{
                            padding: '5px 11px', borderRadius: '7px', fontSize: '0.75rem', fontWeight: 600,
      ...
      {/* Add Company */}
      {modal === 'addCompany' && (
        <Modal title="🏢 Add New Company" onClose={closeModal}>
          {msg.text && <Alert type={msg.type}>{msg.text}</Alert>}
          <form onSubmit={submitCompany} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <Input label="COMPANY NAME *" placeholder="Infosys" value={coForm.name} onChange={e => setCoForm({ ...coForm, name: e.target.value })} required />
            <Select label="INDUSTRY" value={coForm.industry} onChange={e => setCoForm({ ...coForm, industry: e.target.value })}>
              <option value="">Select</option>
              {['IT', 'Finance', 'Healthcare', 'Manufacturing', 'Consulting'].map(i => <option key={i}>{i}</option>)}
            </Select>
            <Input label="LOCATION" placeholder="Bangalore" value={coForm.location} onChange={e => setCoForm({ ...coForm, location: e.target.value })} />
            <Input label="HR EMAIL" type="email" placeholder="hr@co.com" value={coForm.hrEmail} onChange={e => setCoForm({ ...coForm, hrEmail: e.target.value })} />
            <div style={{ gridColumn: '1/-1' }}>
              <Input label="WEBSITE" placeholder="https://company.com" value={coForm.website} onChange={e => setCoForm({ ...coForm, website: e.target.value })} />
            </div>
      ...
      {/* Add Job Role */}
      {modal === 'addJob' && (
        <Modal title="💼 Add Job Role" onClose={closeModal}>
          {msg.text && <Alert type={msg.type}>{msg.text}</Alert>}
          {companies.length === 0 && <Alert type="error">⚠️ Add a company first!</Alert>}
          <form onSubmit={submitJob} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div style={{ gridColumn: '1/-1' }}>
              <Select label={`COMPANY * (${companies.length} available)`} value={jbForm.companyId} onChange={e => setJbForm({ ...jbForm, companyId: e.target.value })} required>
                <option value="">-- Select Company --</option>
                {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </Select>
            </div>
            <Input label="JOB TITLE *" placeholder="Software Engineer" value={jbForm.title} onChange={e => setJbForm({ ...jbForm, title: e.target.value })} required />
            <Input label="SALARY (LPA)" type="number" placeholder="12" value={jbForm.salaryLpa} onChange={e => setJbForm({ ...jbForm, salaryLpa: e.target.value })} />
            <Input label="MIN CGPA" type="number" step="0.1" placeholder="7.0" value={jbForm.minCgpa} onChange={e => setJbForm({ ...jbForm, minCgpa: e.target.value })} />
            <Input label="MIN PASSOUT YEAR" type="number" placeholder="2025" value={jbForm.minPassoutYear} onChange={e => setJbForm({ ...jbForm, minPassoutYear: e.target.value })} />
            <div style={{ gridColumn: '1/-1' }}>
      ...
      {/* Add Skill to Job */}
      {modal === 'addJobSkill' && (
        <Modal title="🔧 Add Required Skill to Job" onClose={closeModal}>
          {msg.text && <Alert type={msg.type}>{msg.text}</Alert>}
          {skills.length === 0 && <Alert type="error">⚠️ Add skills first in the Skills tab!</Alert>}
          <form onSubmit={submitJobSkill} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <Select label={`JOB ROLE * (${jobs.length} available)`} value={jsForm.jobId} onChange={e => setJsForm({ ...jsForm, jobId: e.target.value })} required>
              <option value="">-- Select Job --</option>
              {jobs.map(j => <option key={j.id} value={j.id}>{j.title} — {j.company?.name}</option>)}
            </Select>
            <Select label={`SKILL * (${skills.length} available)`} value={jsForm.skillId} onChange={e => setJsForm({ ...jsForm, skillId: e.target.value })} required>
              <option value="">-- Select Skill --</option>
              {skills.map(s => <option key={s.id} value={s.id}>{s.name}{s.category ? ` (${s.category})` : ''}</option>)}
            </Select>
            <Input label="WEIGHTAGE % (all skills should total 100)" type="number" min="1" max="100" placeholder="30" value={jsForm.weightage} onChange={e => setJsForm({ ...jsForm, weightage: e.target.value })} required />
      ...
      {/* Add Skill (modal from quick action) */}
      {modal === 'addSkill' && (
        <Modal title="🔧 Add New Skill" onClose={closeModal}>
          {msg.text && <Alert type={msg.type}>{msg.text}</Alert>}
          <form onSubmit={submitSkill} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <Input label="SKILL NAME *" placeholder="e.g. Java, React" value={skForm.name} onChange={e => setSkForm({ ...skForm, name: e.target.value })} required />
            <Select label="CATEGORY" value={skForm.category} onChange={e => setSkForm({ ...skForm, category: e.target.value })}>
              <option value="">Select Category</option>
              {['Programming', 'Database', 'Framework', 'Soft Skill', 'Cloud', 'DevOps', 'Other'].map(c => <option key={c}>{c}</option>)}
            </Select>
      ...
      {modal === 'viewStudent' && selStudent && (
        <Modal title={`👨‍🎓 ${selStudent.fullName}`} onClose={closeModal}>
          {msg.text && <Alert type={msg.type}>{msg.text}</Alert>}
          ...
          <div style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(139,92,246,0.18)', borderRadius: '12px', padding: '14px', marginBottom: '18px' }}>
            ...
            <div style={{ marginTop: '10px' }}>
              <Pill color={levelColor[selStudent.readinessLevel] || '#64748b'}>{selStudent.readinessLevel?.replace('_', ' ') || 'N/A'}</Pill>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '4px', background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '4px', marginBottom: '16px' }}>
            {[['skills', '🔧 Skills'], ['certs', '🏅 Certs'], ['internships', '🏭 Internships']].map(([id, label]) => (
              <button key={id} onClick={() => setStuTab(id)} style={{
                flex: 1, padding: '7px', borderRadius: '7px', fontSize: '0.8rem', fontWeight: 700,
                border: 'none', cursor: 'pointer',
                color: stuTab === id ? '#fff' : '#64748b',
                background: stuTab === id ? 'linear-gradient(135deg,#7c3aed,#a855f7)' : 'transparent',
              }}>{label}</button>
            ))}
          </div>
            ...
            <div>
              ...
              <form onSubmit={addStudentSkill} style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div style={{ flex: 1, minWidth: '130px' }}>
                  <Select label="SKILL" value={skillForm.skillId} onChange={e => setSkillForm({ ...skillForm, skillId: e.target.value })} required>
                    <option value="">-- Select --</option>
                    {skills.map(sk => <option key={sk.id} value={sk.id}>{sk.name}</option>)}
                  </Select>
                </div>
                <div style={{ flex: 1, minWidth: '120px' }}>
                  <Select label="PROFICIENCY" value={skillForm.proficiency} onChange={e => setSkillForm({ ...skillForm, proficiency: e.target.value })}>
          ...
          {stuTab === 'certs' && (
            <div>
              {stuCerts.map(c => (
                <div key={c.id} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '8px', padding: '8px 12px', marginBottom: '8px', fontSize: '0.83rem' }}>
                  🏅 <strong>{c.certName}</strong> — {c.issuedBy} ({c.issueYear})
                </div>
              ))}
              ...
              <form onSubmit={addStudentCert} style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <Input placeholder="Cert Name" value={certForm.certName} onChange={e => setCertForm({ ...certForm, certName: e.target.value })} required style={{ flex: '1 1 130px' }} />
                <Input placeholder="Issued By" value={certForm.issuedBy} onChange={e => setCertForm({ ...certForm, issuedBy: e.target.value })} style={{ flex: '1 1 110px' }} />
                <Input placeholder="Year" value={certForm.issueYear} onChange={e => setCertForm({ ...certForm, issueYear: e.target.value })} style={{ flex: '0 0 70px' }} />
                <AdminBtn>Add</AdminBtn>
              </form>
            </div>
          )}
          ...
          {stuTab === 'internships' && (
            <div>
              {stuInts.map(i => (
                <div key={i.id} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '8px', padding: '8px 12px', marginBottom: '8px', fontSize: '0.83rem' }}>
                  🏭 <strong>{i.role}</strong> at {i.companyName} · {i.duration}
                </div>
              ))}
              ...
              <form onSubmit={addStudentInt} style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <Input placeholder="Company" value={intForm.companyName} onChange={e => setIntForm({ ...intForm, companyName: e.target.value })} required style={{ flex: '1 1 130px' }} />
                <Input placeholder="Role" value={intForm.role} onChange={e => setIntForm({ ...intForm, role: e.target.value })} style={{ flex: '1 1 100px' }} />
                <Input placeholder="Duration" value={intForm.duration} onChange={e => setIntForm({ ...intForm, duration: e.target.value })} style={{ flex: '0 0 85px' }} />
                <AdminBtn>Add</AdminBtn>
              </form>
            </div>
          )}
          ...
          <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            <button onClick={() => handleDeleteStudent(selStudent.id)} style={{
              padding: '8px 18px', borderRadius: '8px', fontWeight: 700, fontSize: '0.83rem',
              background: 'rgba(239,68,68,0.1)', color: '#f87171',
              border: '1px solid rgba(239,68,68,0.2)', cursor: 'pointer',

...
export default function CompanyManagement() {
  ...
  const load = async () => {
    try {
      const [c,j,s] = await Promise.all([getAllCompanies(), getAllJobs(), getAllSkills()])
      setCompanies(c.data||[]); setJobs(j.data||[]); setSkills(s.data||[])
    } catch(err) { console.error(err) }
  }

  const openModal = async (type) => {
    setShowModal(type); setMsg({ text:'', type:'' })
    try {
      const [c,j,s] = await Promise.all([getAllCompanies(), getAllJobs(), getAllSkills()])
      setCompanies(c.data||[]); setJobs(j.data||[]); setSkills(s.data||[])
    } catch(err) {}
  ...
  const submitCompany = async (e) => {
    e.preventDefault()
    try {
      await addCompany(coForm)
      setMsg({ text:'✅ Company added!', type:'success' }); setShowModal(null)
      setCoForm({ name:'', industry:'', website:'', location:'', hrEmail:'' }); load()
  ...
  const submitJob = async (e) => {
    ...
    try {
      ...
      setMsg({ text:'✅ Job role added!', type:'success' }); setShowModal(null)
      setJbForm({ companyId:'', title:'', salaryLpa:'', minCgpa:'', minPassoutYear:'', description:'' }); load()
    } catch(err) { setMsg({ text:'❌ '+(err.response?.data?.message||err.response?.data||'Failed'), type:'error' }) }
  }
  ...
  const submitJobSkill = async (e) => {
    ...
    try {
      ...
      setMsg({ text:'✅ Skill added to job!', type:'success' }); setShowModal(null)
      setJsForm({ jobId:'', skillId:'', weightage:'' }); load()
    } catch(err) { setMsg({ text:'❌ '+(err.response?.data?.message||err.response?.data||'Failed'), type:'error' }) }
  }
  ...
  const submitSkill = async (e) => {
    ...
    try {
      ...
      setMsg({ text:'✅ Skill "'+skForm.name+'" added!', type:'success' })
      setSkForm({ name:'', category:'' }); const s=await getAllSkills(); setSkills(s.data||[])
      ...
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'20px', flexWrap:'wrap', gap:'12px' }}>
        <div style={{ display:'flex', gap:'8px', background:'rgba(255,255,255,0.04)', borderRadius:'10px', padding:'4px' }}>
          {['companies','jobs','skills'].map(v=>(
            <button key={v} type="button" onClick={()=>setView(v)} style={{ padding:'8px 18px',borderRadius:'8px',fontWeight:600,fontSize:'0.85rem',background:view===v?'var(--blue)':'transparent',color:view===v?'#fff':'#64748b',transition:'all 0.2s',border:'none',cursor:'pointer' }}>
              {v==='companies'?'🏢 Companies':v==='jobs'?'💼 Job Roles':'🔧 Skills'}
            </button>
          ))}
        </div>
        <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
          <button type="button" className="btn btn-primary" onClick={()=>openModal('company')}>+ Add Company</button>
          <button type="button" className="btn btn-success" onClick={()=>openModal('job')}>+ Add Job Role</button>
          <button type="button" className="btn btn-outline" onClick={()=>openModal('jobskill')}>+ Skill to Job</button>
        </div>
      </div>
        ...
        <div className="tbl-wrap"><table>
          ...
          <tbody>
            {jobs.map(j=><tr key={j.id}>
              ...
              <td style={{ color:'var(--green)',fontWeight:700 }}>₹{j.salaryLpa||0} LPA</td>
              <td>{j.minCgpa}</td>
              <td><div className="skill-chips">{(j.requiredSkills||[]).map(rs=><span key={rs.id} className="chip chip-green">{rs.skill?.name} ({rs.weightage}%)</span>)}{(!j.requiredSkills||j.requiredSkills.length===0)&&<span style={{ color:'#64748b',fontSize:'0.8rem' }}>None</span>}</div></td>
            </tr>)}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'320px 1fr', gap:'20px' }}>
          <div className="card">
            ...
            <div style={{ marginTop:'20px' }}>
              <div style={{ fontSize:'0.78rem',color:'#64748b',marginBottom:'10px' }}>⚡ Quick Add:</div>
              <div style={{ display:'flex', flexWrap:'wrap', gap:'6px' }}>
                {['Java','SQL','DSA','Spring Boot','Python','React','Docker','AWS','Communication','HTML','Microservices','System Design'].map(sk=>(
                  <button key={sk} type="button" onClick={async()=>{ try{await addSkill({name:sk,category:'Programming'});const s=await getAllSkills();setSkills(s.data||[]);setMsg({text:`✅ "${sk}" added!`,type:'success'})}catch(e){setMsg({text:`ℹ️ "${sk}" already exists`,type:'error'})} }}
      ...
      {showModal==='jobskill' && (
        <div className="modal-overlay" onClick={()=>setShowModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            {skills.length===0&&<div className="alert alert-error">⚠️ Add skills first in the 🔧 Skills tab!</div>}

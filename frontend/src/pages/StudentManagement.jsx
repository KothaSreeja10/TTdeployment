...
export default function Attendance() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [s, sk] = await Promise.all([getAllStudents(), getAllSkills()])
      setStudents(s.data || [])
      setSkills(sk.data || [])
    } catch(e) {}
  ...
  const badge = (level) => {
    ...
    return <span className={`badge ${map[level]||'badge-slate'}`}>{level || 'N/A'}</span>
  }
        ...
        <div className="card" style={{padding:0,overflow:'hidden'}}>
          <div style={{padding:'14px 20px',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            ...
            <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
              <span className="badge badge-green">High: {students.filter(s=>s.readinessLevel==='HIGH').length}</span>
              <span className="badge badge-orange">Medium: {students.filter(s=>s.readinessLevel==='MEDIUM').length}</span>
              <span className="badge badge-red">Low: {students.filter(s=>s.readinessLevel==='LOW').length}</span>
            </div>
          </div>
          <div style={{maxHeight:'70vh',overflowY:'auto'}}>
            {loading ? (
              [1,2,3].map(n => (
                <div key={n} style={{padding:'14px 18px',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div style={{flex:1}}>
                    <div style={{height:'14px',width:'55%',borderRadius:'6px',background:'rgba(255,255,255,0.08)',marginBottom:'8px',animation:'pulse 1.4s ease-in-out infinite'}} />
                    <div style={{height:'11px',width:'75%',borderRadius:'6px',background:'rgba(255,255,255,0.05)',marginBottom:'8px',animation:'pulse 1.4s ease-in-out infinite'}} />
                    <div style={{display:'flex',gap:'6px'}}>
                      <div style={{height:'20px',width:'60px',borderRadius:'999px',background:'rgba(255,255,255,0.07)',animation:'pulse 1.4s ease-in-out infinite'}} />
                      <div style={{height:'20px',width:'80px',borderRadius:'999px',background:'rgba(255,255,255,0.05)',animation:'pulse 1.4s ease-in-out infinite'}} />
                    </div>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'6px'}}>
                    <div style={{height:'14px',width:'80px',borderRadius:'6px',background:'rgba(255,255,255,0.07)',animation:'pulse 1.4s ease-in-out infinite'}} />
                    <div style={{height:'12px',width:'70px',borderRadius:'6px',background:'rgba(255,255,255,0.05)',animation:'pulse 1.4s ease-in-out infinite'}} />
                  </div>
                </div>
              ))
            ) : (
                ...
                <div key={s.id} onClick={()=>selectStudent(s)} style={{padding:'14px 18px',borderBottom:'1px solid var(--border)',cursor:'pointer',display:'flex',justifyContent:'space-between',background:selected?.id===s.id?'rgba(37,99,235,0.1)':'transparent',borderLeft:selected?.id===s.id?'3px solid var(--blue)':'3px solid transparent'}}>
                  <div>
                    ...
                    <div style={{marginTop:'6px',display:'flex',gap:'6px',alignItems:'center',flexWrap:'wrap'}}>
                      ...
                      <span style={{fontSize:'0.72rem',fontWeight:700,padding:'2px 8px',borderRadius:'999px',background:['HIGH','MEDIUM'].includes(s.readinessLevel)?'rgba(16,185,129,0.2)':'rgba(239,68,68,0.15)',color:['HIGH','MEDIUM'].includes(s.readinessLevel)?'var(--green)':'var(--red)',border:['HIGH','MEDIUM'].includes(s.readinessLevel)?'1px solid rgba(16,185,129,0.4)':'1px solid rgba(239,68,68,0.3)'}}>
                        {['HIGH','MEDIUM'].includes(s.readinessLevel)?'✅ Ready':'❌ Not Ready'}
                      </span>
                    </div>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'6px'}}>
            ...
            {!loading && students.length===0 && <p style={{padding:'20px',color:'#64748b'}}>No students yet.</p>}
          </div>
        </div>
        ...
        {selected && (
          <div className="card">
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:'16px'}}>
              <div><h2 style={{color:'#fff',fontWeight:800}}>{selected.name}</h2><p style={{color:'#64748b',fontSize:'0.83rem'}}>{selected.email}</p></div>
            ...
            {detailLoading && (
              <div style={{textAlign:'center',padding:'24px',color:'#64748b'}}>
                <div style={{display:'inline-block',width:'28px',height:'28px',border:'3px solid rgba(255,255,255,0.1)',borderTop:'3px solid var(--blue)',borderRadius:'50%',animation:'spin 0.8s linear infinite',marginBottom:'8px'}} />
                <p style={{fontSize:'0.83rem'}}>Loading student data…</p>
              </div>
            )}

...
export default function StudentDashboard() {
  ...
  const load = async () => {
    const [scoreRes, skillsRes, certsRes, intsRes, jobsRes] = await Promise.allSettled([
      getReadinessScore(user.id),
      getStudentSkills(user.id),
      getCertifications(user.id),
      getInternships(user.id),
      getAllJobs(),
    ])
    ...
    <div className="page">
      ...
      <div className="card" style={{ marginBottom:'24px', background:'linear-gradient(135deg, rgba(37,99,235,0.15), rgba(6,182,212,0.1))' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'16px' }}>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:'0.82rem', color:'#64748b', marginBottom:'6px', textTransform:'uppercase', letterSpacing:'0.06em' }}>
              Your Placement Readiness Score
            </div>
            <div style={{ fontSize:'3rem', fontWeight:900, color: levelColor[level], lineHeight:1 }}>
          ...
          <div style={{ background:'rgba(0,0,0,0.2)', borderRadius:'12px', padding:'16px', minWidth:'220px' }}>
            ...
            {neededFor60 > 0 && (
              <div style={{ fontSize:'0.8rem', color:'var(--orange)', marginBottom:'6px' }}>
                → Add {neededFor60} more skill{neededFor60 > 1 ? 's' : ''} to reach JOB READY
              </div>
            )}
            {neededFor80 > 0 && score >= 60 && (
              <div style={{ fontSize:'0.8rem', color:'var(--cyan)', marginBottom:'6px' }}>
                → Add {neededFor80} more skill{neededFor80 > 1 ? 's' : ''} for HIGHLY COMPETITIVE
              </div>
            )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginBottom:'20px' }}>
        <div className="card">
          ...
          {skills.length === 0
            ? (
              <div>
                <p style={{ color:'#64748b', fontSize:'0.88rem', marginBottom:'12px' }}>No skills added yet.</p>
                <button className="btn btn-primary btn-sm" onClick={() => navigate('/attendance')}>+ Add Skills</button>
              </div>
            )
        ...
        <div className="card">
          ...
          {certs.length === 0
            ? (
              <div>
                <p style={{ color:'#64748b', fontSize:'0.88rem', marginBottom:'12px' }}>No certifications yet.</p>
                <button className="btn btn-primary btn-sm" onClick={() => navigate('/attendance')}>+ Add Certification</button>
              </div>
            )
            : certs.map(c => (
              <div key={c.id} style={{ background:'rgba(255,255,255,0.04)', borderRadius:'8px', padding:'8px 12px', marginBottom:'8px', fontSize:'0.85rem' }}>
